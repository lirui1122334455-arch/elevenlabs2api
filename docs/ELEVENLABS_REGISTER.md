...
`elevenlabs-register` 是独立的 Playwright sidecar，一次只运行一个注册任务。它与共享运行配置卷配合，使用 YYDS 自有域名邮箱，并可选择 YesCaptcha 或 Captcha Gateway 完成 ElevenLabs 邮件注册流程。
...
代理留空时使用直连，不会读取系统代理或其他容器端口。填写代理时支持 HTTP、HTTPS、SOCKS4、SOCKS4A、SOCKS5 和 SOCKS5H，日志只显示脱敏后的 scheme、host 和 port。
...
| `POST` | `/v1/register` | 创建邮箱、调用所选打码供应商并提交真实注册 |
