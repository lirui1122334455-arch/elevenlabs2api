...
class YesCaptchaSolver:
    ...
    def _http_session(self) -> requests.Session:
        """Captcha API should not inherit socks ALL_PROXY from the shell."""
    ...
    def _post_json(self, path: str, payload: dict, *, timeout: float = 30.0) -> dict:
        ...
        try:
            ...
            # Some compatible hosts return 400 with JSON error body for bad task type.
            ...
            if not self._auto_fallback_endpoint:
                raise
            # Network / DNS / TLS issues: try the other region once (YesCaptcha only)
            peer = None
            ...
            self._endpoint = peer
            resp = session.post(f"{self._endpoint}{path}", json=payload, timeout=timeout)
            resp.raise_for_status()
            data = resp.json()
            if not isinstance(data, dict):
                raise RuntimeError(f"YesCaptcha non-object response: {data!r}")
    ...
    def solve_turnstile(
        ...
        *,
        ...
        # YesCaptcha-only premium type; 2Captcha / Anti-Captcha-compatible
        # hosts only support TurnstileTaskProxyless.
        # ez-captcha uses CloudFlareTurnstileTask (YesCaptcha-compatible wire API).
        ep = (self._endpoint or "").lower()
        is_ez = "ez-captcha.com" in ep or "ezcaptcha" in ep
        supports_m1 = ("yescaptcha.com" in ep) or not any(
            x in ep
            for x in (
                "2captcha.com",
                "anti-captcha.com",
        ...
        if not supports_m1:
            premium = False
            fallback_non_premium = False

        task_types: list[str] = []
        if is_ez:
            # Primary + a few aliases some ez-captcha panels expose.
            task_types.extend(
                [
            ...
            task_types.append("TurnstileTaskProxyless")
            if fallback_non_premium and supports_m1:
                task_types.append("TurnstileTaskProxylessM1")
