...
class XConsoleAuthClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._last_session_diagnostics: Dict[str, object] = {}
    ...
    def cookie_metadata(self) -> List[Dict[str, str]]:
        """Return cookie names and scopes without exposing cookie values."""
        cookies = self._t.cookies
        ...
        try:
            for cookie in jar:
                ...
                if name and key not in seen:
                    seen.add(key)
                    items.append({"name": name, "domain": domain, "path": path})
        except TypeError:
            for name in self.cookie_names():
                key = (str(name), "", "")
                if key not in seen:
                    seen.add(key)
                    items.append({"name": str(name), "domain": "", "path": ""})
        return items

    def session_diagnostics(self) -> Dict[str, object]:
        """Return the latest password-session result without credentials or tokens."""
        return dict(self._last_session_diagnostics)
    ...
    def obtain_session_via_password(
        ...
        *,
        ...
        for attempt in range(1, attempts + 1):
            for em in email_candidates:
                ...
                for raw_cookie in set_cookies or []:
                    match = re.match(r"\s*([^=;,\s]+)=", str(raw_cookie))
                    if match and match.group(1) not in set_cookie_names:
                        set_cookie_names.append(match.group(1))
                if self.debug:
                    print(
                ...
                self._last_session_diagnostics = {
                    "attempted": True,
                    "attempt": attempt,
                    "email_variant": "original" if em == email_raw else "lowercase",
                    "http_status": status,
                    "grpc_status": grpc_status,
                    "grpc_message": (grpc_msg or "")[:200],
                ...
                if session_jwt:
                    try:
                        ...
                        pass
                    self._last_session_diagnostics["ok"] = True
                    return session_jwt
        ...
        self._last_session_diagnostics.setdefault("ok", False)
        return self._read_sso_from_jar()
